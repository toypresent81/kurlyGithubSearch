...
8.
다음의 버그가 있어서 같이 고민해서 수정하고 싶어
-검색 화면에서 검색된 리스트를 통해 결과화면으로 진입했어
-결과화면에는 검색어가 서치바에 노출되고 클리어버튼도 같이 노출된 상태야
-클리어버튼 탭시 최근 검색화면이 노출되야 하는데 서치바에 포커싱이 가면서 취소버튼이 나타나는 상황이야
-클리어버튼 탭시에도 취소버튼 누르면 이전 검색화면이 나타나는것처럼 개선하고 싶은데 어디가 문제인지 같이 찾아봐줘
